...
export const eventRegistrationApi = {
  // 注册参加活动
  registerForEvent: async (eventId: number, userId: number, registrationData: NewEventRegistration): Promise<EventRegistration | null> => {
    try {
      const response = await request({
  ...
  // 获取用户注册的所有活动
  getUserEvents: async (userId: number): Promise<Array<{ registration: EventRegistration; event: Event }>> => {
    try {
      const response = await request({
  ...
  // 更新注册信息
  updateRegistration: async (eventId: number, userId: number, registrationData: NewEventRegistration): Promise<EventRegistration | null> => {
    try {
      const response = await request({
        url: `/events/register/${eventId}/${userId}`,
        method: 'PUT',
