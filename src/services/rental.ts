...
export const rentalApi = {
  // 获取所有租赁商品（支持筛选和分页）
  getAllItems: async (filters?: RentalFilters): Promise<PaginatedRentalResponse> => {
    try {
      const queryParams = new URLSearchParams();

      if (filters?.priceFrom !== undefined)
        queryParams.append('priceFrom', filters.priceFrom.toString());
      if (filters?.priceTo !== undefined) queryParams.append('priceTo', filters.priceTo.toString());
      ...
      const url = queryParams.toString() ? `/rental?${queryParams.toString()}` : '/rental';
  ...
  getCategories: async (): Promise<RentalCategory[]> => {
    try {
      ...
      if (response && response.data && Array.isArray(response.data)) {
        return response.data.map((cat: { id: string; name: string }, index: number) => ({
          id: index + 1,
          name: cat.name,
