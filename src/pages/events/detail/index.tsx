...
const EventDetail: React.FC = () => {
  ...
  // Handle copy WeChat ID
  const handleCopyWechat = () => {
    const wechatId = 'Brisbane10000';
  ...
  if (loading) {
    return (
      <View className="event-detail-page loading-state" style={{ paddingTop: '200rpx' }}>
        <Text style={{ color: '#666' }}>正在加载精彩活动...</Text>
      </View>
    );
  }
  ...
  if (errorMsg || !event) {
    return (
      <View
        className="event-detail-page error-state"
        style={{ paddingTop: '200rpx', flexDirection: 'column' }}
      >
        <Text style={{ fontSize: '48rpx', marginBottom: '20rpx' }}>⚠️</Text>
        <Text style={{ color: '#666', marginBottom: '40rpx' }}>{errorMsg || '未找到活动信息'}</Text>
        <View
          onClick={handleBack}
          style={{
            padding: '20rpx 60rpx',
  ...
  return (
    <View className="event-detail-page">
      <View className="event-detail-scroll">
        ...
        <View className="content-wrapper">
          ...
          <View className="detail-card description-card">
            ...
            <Text className="description-text">{event.description || '暂无更多详细介绍。'}</Text>
          </View>

          {/* Contact Info */}
          <View className="detail-card contact-card">
            <Text className="section-title">联系方式</Text>
            <View className="contact-info" onClick={handleCopyWechat}>
              <Text className="contact-icon">💬</Text>
              <View className="contact-details">
                <Text className="contact-label">微信号</Text>
      ...
      <View className="floating-action-bar">
        <View className="action-btn share-btn" onClick={handleCopyWechat}>
          📤
        </View>
        {!isPastEvent && (
          <View className="action-btn primary-btn" onClick={handleCopyWechat}>
