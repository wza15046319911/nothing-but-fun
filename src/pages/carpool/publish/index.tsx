...
import { DatePicker, Button, Picker, Toast } from '@nutui/nutui-react-taro';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditMode && editingId) {
        const result = await carpoolApi.updateCarpoolByOpenId(
          userInfo.openid,
          editingId,
          carpoolData
        );
    ...
    <View className="carpool-publish-container">
      ...
      <View className="form-container">
        ...
        <View className="quick-row">
          ...
          <Button size="small" className="swap-route" onClick={swapRoute}>
            ⇄ 互换
          </Button>
        </View>
        {/* Route Row */}
        ...
        <View className="form-section">
          ...
          <View className={`form-field ${errors.departureTime ? 'error' : ''}`}>
            ...
            <View className="picker-field" onClick={() => setShowDatePicker(true)}>
        ...
        <View className="form-section">
          ...
          <View className="form-row two-cols">
            <View className={`form-field ${errors.availableSeats ? 'error' : ''}`}>
              ...
              <View className="picker-field" onClick={() => setShowSeatsPicker(true)}>
                <Text className="picker-value">{availableSeats}个座位</Text>
                <Text className="picker-icon">▼</Text>
              </View>
              {errors.availableSeats && (
                <Text className="field-error">{errors.availableSeats}</Text>
              )}
        ...
        <View className="form-section">
          ...
          <View className="form-field switch-field">
            ...
            <View className="switch-wrapper">
              ...
              <Switch checked={insured} onChange={(e) => setInsured(e.detail.value)} />
            </View>
          </View>
        </View>
      </View>
      ...
      <View className="action-buttons">
        <Button type="default" className="cancel-button" onClick={handleCancel}>
          取消
        </Button>
        <Button type="primary" className="submit-button" onClick={handleSubmit} loading={loading}>
          {loading
            ? isEditMode
              ? '保存中...'
              : '发布中...'
