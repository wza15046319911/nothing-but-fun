...
import {
  rentalHouseApi,
  RentalHouse as RentalHouseType,
  RentalHouseQueryParams,
} from '../../services/rental_house';
import './index.less';
...
const RentalHouse: React.FC = () => {
  ...
  const getFeatureTags = (features: string[]) => {
    ...
    return features.slice(0, 3).map((feature) => featureMap[feature] || feature);
  };
      ...
      <PullToRefresh onRefresh={handleRefresh}>
        <ScrollView className="content" scrollY>
          {loading ? (
            ...
            <Empty description="暂无房源信息" imageSize={120} />
          ) : (
            <View className="houses-list">
              {houses.map((house) => (
                <View key={house.id} className="house-card" onClick={() => handleHouseClick(house)}>
                  {/* 房源图片 */}
                  ...
                  <View className="house-info">
                    ...
                    <View className="house-location">
                      <Text className="location-text">
                        📍 {house.streetAddress}, {house.suburb}
                      </Text>
                    </View>
